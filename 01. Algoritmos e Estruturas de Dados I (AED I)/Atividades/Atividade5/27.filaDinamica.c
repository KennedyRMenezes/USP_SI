...
bool inserirNaFila(FILA* f,REGISTRO reg) {
  ...
  novo->reg = reg;
  novo->prox = NULL;
  if (f->inicio==NULL) f->inicio = novo; //Se for o primeiro elemento 
...
bool excluirDaFila(FILA* f, REGISTRO* reg) {
  ...
  *reg = f->inicio->reg; //copia o registro do 1º elem. Será sempre o 1º a ser excluído.
  PONT apagar = f->inicio; //variável auxilar p/ guardar o end do elem. que vai apagar.
  f->inicio = f->inicio->prox; //inicio será o posterior do elem. que está sendo excluído.
  free(apagar);
  if (f->inicio == NULL) f->fim = NULL; //se o elem. excluido era o unico, inicio fica como NULL e o fim deve ficar como NULL tmb.
  return true;
} /* excluirDaFila */
