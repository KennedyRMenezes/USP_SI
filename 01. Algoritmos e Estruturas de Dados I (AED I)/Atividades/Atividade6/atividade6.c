...
/* Inicializa��o do deque (o deque jah esta criado e eh apontado 
pelo endereco em d) */
...
/* Exibi��o do DEQUE a partir do inicio1 */
void exibirDequeInicio(DEQUE* d){
  PONT end = d->cabeca->prox;
  printf("Deque partindo do in�cio: \" ");
  while (end != d->cabeca){
...
/* Exibi��o do DEQUE a partir do inicio2 */
...
/* Exclus�o do primeiro elemento, entrada 1 do deque 
...
/* Exclus�o do primeiro elemento, entrada 2 do deque
   e colocar o valor da chave do elemento memoria apontada por ch*/
...
/* Reinicializa��o/Destrui��o do deque
...
bool encontrarMax(DEQUE* d, int* max) {
  bool resposta = false;
  if (d->cabeca->prox == d->cabeca) return resposta;//deque vazio
  ...
  *max = end->reg.chave;
  while (end != d->cabeca){
    if(end->reg.chave > *max){
      *max = end->reg.chave; //retorna maior elemento para o endereço de max.
    }
    end = end->prox;
  }
  
  resposta = true; //achou o maior elemento, muda para true
  return resposta; //retorna true
}
