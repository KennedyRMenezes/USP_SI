...
PONT rotacaoL(PONT p){
	...
	if(u->bal == -1) {   // LL
		...
		v->bal = 0;
		return v;
	}else{   // caso necessario apenas para a exclusao (u->bal == 0) L0
		p->esq = u->dir;
		u->dir = p;
...
void rotacaoL2(PONT *p){
	...
	(*p)->bal = 0; // balan�o final da raiz (p) da subarvore
}
...
/* Inser��o AVL2: p � inicializado com raiz e *alterou com false */
