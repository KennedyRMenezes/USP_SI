...
PONT removerElemento(PFILA f){
  ...
  int indNovaRaiz = tamanho(f)-1; //Ultimo elem. serah a nova raiz
  int idElemExcl = f->heap[0]->id; //Guardando id do elem. excluido
  PONT elemExcl = f->heap[0]; //Guardando elem. excluido
  f->heap[0] = f->heap[indNovaRaiz]; //Colocando nova raiz no lugar
  max_heapify(f, 0, tamanho(f)); //Reorganizando heap
...
//Inserir e aumentar possuem a mesma logica para organizar os elementos

//Excluir e reduzir possuem a mesmma logica para organizar os elementos